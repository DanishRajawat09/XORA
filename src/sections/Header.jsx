        ...
        <header className= {clsx('fixed top-0 left-0 z-50 w-full py-10 transition-all duration-500 max-lg:py-4' , hasScroll && "py-2 bg-black-100 backdrop-blur[8px] ")}>
            <div className='container flex h-14 items-center max-lg:px-5'>
                <a className='lg:hidden flex-1  z-2'>
                    <img className='cursor-pointer' src="/images/xora.svg" alt="logo" width={115} height={55} />
                </a>

                <div className={clsx(
                    'w-full max-lg:fixed max-lg:top-0 max-lg:left-0 max-lg:w-full max-lg:bg-s2 ', magic ? "max-lg:opacity-100" : "max-lg:opacity-0 max-lg:pointer-events-none "
                )} >
                ...
                <button className='lg:hidden z-2 size-10 border-2 border-s4/25 rounded-full flex justify-center items-center '
                    onClick={(e) => setMagic((prev) => !prev)}
                >
                    <img src={`/images/${magic ? "close" : "magic"}.svg`} alt="magic" className='size-1/2 object-contain' /></button>
            </div>
        </header>
    )
}
